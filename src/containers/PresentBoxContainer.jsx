...
const CheckBox =(item) =>{
		return (<Paper style={style} key={item.id}
			       ...
			       <Checkbox id={item.id} 
			      				...
			      				checkedIcon={<CheckCircle color={green400} style={{width: 48,height: 48,padding: 24,}}/>}
			      				uncheckedIcon={
			      				<SvgIcon color={red500} style={{width: 48,height: 48,marginLeft:-10,marginTop:-10}}>
...
const PresentBoxContainer = React.createClass( {
	...
	render: function(){
		...
		if (items.length===0) {
			return(<h1>Add new hours for this day!</h1>);
	    }
		else return (
			<div>
				{items}			
